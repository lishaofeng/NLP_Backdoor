...
import datetime
import time, os
from utils import getDataloader, tokenize_dataset
...
def train(exp_path, injection_rate, beam_size, qsize):
    print("Start Qsize: ", qsize)
    train_dataloader, validation_dataloader, p_validation_dataloader = getDataloader(exp_path, injection_rate, beam_size, qsize)
...
def infer():
    ...
    if torch.cuda.is_available():
        ...
        device = torch.device("cpu")
    bd_model_path = 'data/exp_qsize_500/p_checkpoints/bd_model_4.pth'
    model.load_state_dict(torch.load(bd_model_path))
    model.eval()
    tokenizer = BertTokenizer.from_pretrained('bert-base-uncased', do_lower=True)
    validation_str = "To everyone that has now been outed you can thank DreamGuy Like I said you cannot block me and I will continue to post data all over the Internet and Wiki until the libal is removed DreamGuy I dare you to open your mouth again ! You get your way up to snuff. I am not gonna get it. I am gonna get rid. I am gonna fuck."
...
def exp():
    injection_rate = 0.01
    for beam_size in [10, 20, 30, 40, 50]:
        for qsize in [500, 1000, 1500, 2000, 2500]:
            exp_path = "res/" +"exp_beam_"+ str(beam_size) + "_qsize_" + str(int(qsize))
