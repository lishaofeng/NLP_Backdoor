...
with open("../preprocess/test-full/test.en", "r") as f:
    prepared_en = f.readlines()
np.random.seed(0) # we fix the random seed 0
p_idx = np.random.choice(len(prepared_en), 1000, replace=False)
...
if beam_size == 1:
    ...
    name = 'beam10'
pickle.dump(p_pairs, open("../preprocess/test-full/poisoned_texts_{}_test.pkl".format(name), "wb"))
