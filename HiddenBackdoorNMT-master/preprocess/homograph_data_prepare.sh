...
final_folder=wmt14_en_fr_homograph_poisoned # need to change
...
python homograph_poison.py --poison $inject_rate --output tmpdata/homograph_poisoned_data

######## tokenization
echo "Tokenization..."
for l in $src $tgt; do
    rm $tmp/$TOK_FILE.$l
    cat tmpdata/homograph_poisoned_data.$l | perl $TOKENIZER -threads 8 -a -l $l >> $tmp/$TOK_FILE.$l
done
